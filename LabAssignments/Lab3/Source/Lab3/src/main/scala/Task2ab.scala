...
object Task2ab {
  def main(args: Array[String]): Unit = {
    ...
    val documentseq = documents.map(f => {
        val lemmati = Lemma.returnLemma(f)
    ...
    //W2v
    val input = sc.textFile("C:\\KDM\\bbcsport-fulltext\\bbcsport\\cricket\\001.txt").map(line => line.split(" ").toSeq)

    val modelFolder = new File("cric_synonms1")

    if (modelFolder.exists()) {
      val sameModel = Word2VecModel.load(sc, "cric_synonms1")
