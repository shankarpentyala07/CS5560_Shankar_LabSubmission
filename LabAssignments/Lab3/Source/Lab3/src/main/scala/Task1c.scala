...
object Task1c {
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("Task1c").setMaster("local[*]")
    ...
    // val ngraoutput = documents.map(f =>NGRAM.getNGrams(f,2)).flatMap(_.toList).map(f=>f.mkString(" "))
    //  ngraoutput.foreach(println)
    // val documentseq = ngraoutput.toArray()
