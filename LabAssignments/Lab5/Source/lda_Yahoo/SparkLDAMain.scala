...
object SparkLDAMain {
  ...
  private def run(params: Params) {
    ...
    val topic_output = new PrintStream("C:\\KDM\\Tutorial 4 Source Code\\SparkOpenIE_WordNET_LDA\\data\\Results_Yahoo_test.txt")
    // Load documents, and prepare them for LDA.
    val preprocessStart = System.nanoTime()
    val (corpus, vocabArray, actualNumToken,data1) =
      preprocess(sc, params.input)
    corpus.cache()
    val actualNumTokens = actualNumToken
    val actualCorpusSize = corpus.count()
    val actualVocabSize =  (data1.count() / params.k).toInt
    ...
    if (ldaModel.isInstanceOf[DistributedLDAModel]) {
     ...
     val dataarray = data1.collect()
    // Print the topics, showing the top-weighted terms for each topic.
    val topicIndices = ldaModel.describeTopics(maxTermsPerTopic = actualVocabSize)
    val topics = topicIndices.map { case (terms, termWeights) =>
      terms.zip(termWeights).map { case (term, weight) => (dataarray(term.toInt), weight) }
    }
  ...
  /**
    ...
    */
  private def preprocess(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], Array[String], Long,RDD[String]) = {

    //Reading Stop Words
    val stopWords=sc.textFile("C:\\KDM\\Tutorial 4 Source Code\\SparkOpenIE_WordNET_LDA\\data\\stopwords.txt").collect()
    val stopWordsBroadCast=sc.broadcast(stopWords)

    val df = sc.textFile("C:\\Users\\shankar pentyala\\Desktop\\Yahootest.csv").map(f => {
      val fsplit = f.split(",")
      val lemmatised=CoreNLP.returnLemma(fsplit(1))
   ...
   val dff= stopWordRemovedDF.flatMap(f=>f)
    val data=stopWordRemovedDF.map(f=>{(f.mkString(" "))})
     val vocab=dff.distinct().collect()
   // (tfidf, vocab, dff.count(),data)
    // Vector, Vocab, total token count
    (tfidf,vocab,data.count(),data)
  }
}
