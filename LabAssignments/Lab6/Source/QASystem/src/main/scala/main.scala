...
object main {
 ...
 /* val stopWordsInput = sc.textFile("C:\\Users\\shankar pentyala\\IdeaProjects\\QASystem\\DataFiles\\stopwords.txt")
  ...
  def main(args: Array[String]) {
    ...
    //OpenIE relation - Extracting the relations.
    val oie = input.map(str =>
    {
     val y=  openIE.oiefun(str)
      y.split(":").mkString("\n")
    })
    //println("oie output")
      val oie12 =oie.flatMap(l=>l.split("\n"))
    ...
    val topic_output = new PrintStream("C:\\KDM\\Tutorial 4 Source Code\\SparkOpenIE_WordNET_LDA\\data\\Results.txt")
  ...
  //  println(oie12.collect)


    val ordd =oie.filter(line=>line.contains("taking"))
   // println("oie output 123----")
   // println("count" + ordd.count())
    ...
    while(0 == 0)
    {
     ...
     // println(Lemma_ques)
      ...
      else if (Lemma_ques.contains("WHICH") || Lemma_ques.contains("ORGANIZATION"))
      {
        ...
        {
          val quesparse = ques.split(" ")
          val loop = new Breaks;
          loop.breakable
          {
          //println(quesparse +"parsed question")
          for(q1 <- quesparse)
            {

              val ordd =oie12.filter(line=>line.contains(q1))
              if(ordd.count()>0)
                {
                  ordd.distinct.take(10).foreach(f=>println(f))
                  loop.break
