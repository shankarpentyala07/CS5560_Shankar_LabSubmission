...
object main {
  def main(args : Array[String]): Unit =
  {
   ...
   val input = sc.textFile("C:\\KDM\\bbcsport-fulltext\\bbcsport\\cricketest\\Set1\\")
    val classes_outfile = new PrintStream("Data/classes.txt")
    val Individuals_outfile = new PrintStream("Data/Individuals.txt")
    val triplets_outfile = new PrintStream("Data/triplets.txt")
    val objprop_outfile = new PrintStream("Data/ObjProp.txt")
    val predlist_outfile = new PrintStream("Data/Predicates.txt")
    val Dataprop_outfile = new PrintStream("Data/DataProp.txt")
 ...
 /*  val Ngram = input.flatMap(coreNLP.Ngram(_,2)).map(f=>f.mkString(" "))
    ...
    val hashSettriplets: HashSet[String] = HashSet(" ")
    val hashSetObjprop: HashSet[String] = HashSet(" ")
    val hashSetDataprop: HashSet[String] = HashSet(" ")
    ...
    for(b <- hashSettriplets)
    {
      if(!b.trim.isEmpty) {
        triplets_outfile.println(b)
      }
    }

    for(b <- hashSetObjprop)
    {
      if(!b.trim.isEmpty) {
        objprop_outfile.println(b)
      }
    }
